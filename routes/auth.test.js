process.env.NODE_ENV = "test";// set node enviroment to test so we use the test database.
...
const BCRYPT_WORK_FACTOR = 1;// set to one so it doesnt slow the test down.
...
describe('POST /login', function () {
  test("returns logged in msg", async function () {
    ...
    expect(response.statusCode).toBe(200);
    // i want some body(response.body) to equal some object that contains this property
